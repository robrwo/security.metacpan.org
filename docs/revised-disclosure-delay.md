...
At the [Perl Toolchain Summit in April 2026](https://perltoolchainsummit.org/pts2026/), members of the CPANSec CNA discussed responsible disclosure and decided it was time to reduce the _default_ planned disclosure date ("embargo") for publishing vulnerabilities from 28 days to 14 days.

Some CPAN authors have expressed concern about this change, so we wanted to document why we have made this change.
...
The technology has improved and LLMs can produce useful vulnerability reports.
...
Whether we like it or not, the source code of many open source projects have been ingested by LLMs.  People are using those LLMs to discover security issues and other bugs.

In many cases, as with CPANSec, the intention is to attempt to identify, triage and coordinate fixes for security vulnerabilities.

In other cases, attackers are attempting to find vulnerabilities to allow them to access systems and information for which they are not authorized.

LLMs are being used for both reasons.
However, LLMs are being used becasue they can and do find real resurity vulnerabilities.

### Minor issues matter

LLMs have proven useful in chaining together several low severity vulnerabilities into a proof of concept that can lead to full exploitation.
What would previously have required a skilled vulnerability researcher, is now within reach for most attackers.
...
Software bugs have real-world consequences. (FIXME: here we should reference the Barracuda Email Security Gateway https://security.metacpan.org/2024/02/10/vulnerable-spreadsheet-parsing-modules.html) 

### Downstream users matter

Security vulnerabilities have a major impact on downstream users of our software packages.  Unreported or unfixed vulnerabilities can lead to system compromise.

The end user data held in or protected by systems that use vulnerabile components can be disclosed and adversely affect those end users.
...
However, this only front-loads the flood of reports before newer technologies become more widely available.
